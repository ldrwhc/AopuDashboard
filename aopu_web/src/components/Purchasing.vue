<template>
  <div>
    <el-breadcrumb separator="/" style="margin-bottom: 10px">
      ...
      <el-breadcrumb-item>代理商进货数据</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    >
      ...
      <el-table-column prop="purchaseDate" label="进货日期" sortable >
      </el-table-column>
...
<!--    弹出添加表单-->
    <el-dialog title="添加|修改代理商进货数据" :visible.sync="dialogFormVisible" class="demo-ruleForm">
      <el-form :model="form">
        ...
        >
          <el-input v-model.number="form.discount" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods:{
    ...
    updateStore(row){
      ...
      this.form.category = row.storeClass
    ...
    deleteStore(id){
      this.$confirm('确认删除？')
          .then(_ => {
            console.log(id)
            this.$axios({
              url: this.$httpUrl + '/delete/deletePurchasing',
              method: 'get',
              headers: {
    ...
    handleClose() {
      this.$confirm('确认提交？')
          .then(_ => {
            console.log("1111")
            this.$axios({
              url:this.$httpUrl + '/add/addPurchasing',
              method:'post',
              headers: {
                token: localStorage.getItem("token")
              },
    ...
    loadData(){
      this.tableData = JSON.parse(localStorage.getItem("purchasing"));
    },
    ...
    searchByName(){
      ...
      this.searchState = 1;
      this.$axios({
        url:this.$httpUrl+"/find/findAllPurchasing",
        method: 'post',
        headers: {
          token: localStorage.getItem("token")
        },
        params:{
          "purchasingName":this.searchName,
        },
        data:{
        ...
        if(res.data.code === 200){
          ...
          localStorage.setItem("purchasing",JSON.stringify(res.data.data.purchasing))
          this.total = res.data.data.purchasingLength
          this.loadData()
        }
      })
