<template>
  <div>
    ...
    <el-row type="flex" class="row-bg" style="padding-bottom: 5px;font-size: small">
      ...
      <el-button type="primary" style="margin-left:10px;float:right;" @click="exportExcelSelect">导出Excel</el-button>
      <el-upload action="http://localhost:8089/upload/uploadAgent"
                 :on-success="successUpload">
        <el-button type="primary" style="margin-left: 10px;font-size: 12px">点击上传</el-button>
      </el-upload>
    ...
    >
      ...
      <el-table-column prop="agentName" label="代理商姓名"
      ...
      <el-table-column prop="agentBrand" label="代理品牌" sortable>
      </el-table-column>
...
<!--    弹出添加表单-->
    <el-dialog title="添加|修改代理商信息" :visible.sync="dialogFormVisible" class="demo-ruleForm">
      <el-form :model="form">
        ...
        <el-form-item label="合作时间" :label-width="formLabelWidth"
                      prop="openTime"
                      :rules="[
             { required: true, message: '不能为空'},
           ]"
        ...
        >
          <el-input v-model="form.agentBrand" autocomplete="off"></el-input>
        </el-form-item>
...
<!--   导出预览 -->
    <el-dialog title="表格保存预览" width="70%" :visible.sync="selectWindow">
      <el-table :data="selectData" id="selectTable" height="380px">
        ...
        <el-table-column prop="agentBrand" label="代理品牌">
        </el-table-column>
        <el-table-column prop="cooperationTime" label="合作时间">
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods:{
    ...
    updateStore(row){
      ...
      this.form.agentBrand = row.agentBrand
      this.form.cooperationTime = row.cooperationTime
    ...
    deleteStore(id){
      this.$confirm('确认删除？')
          .then(_ => {
            console.log(id)
            this.$axios({
              url: this.$httpUrl + '/delete/deleteAgent',
              method: 'get',
              headers: {
          ...
          .then(_ => {
            ...
            this.$axios({
              ...
              data:{
                ...
                "agentBrand":this.form.agentBrand,
                "cooperationTime":this.form.cooperationTime,
              }
            }).then((res)=> {
              console.log("添加成功")
              if (res.data.code === 200) {
    ...
    findByProvinceAndCity(){
      ...
      this.$axios( {
        url:this.$httpUrl+"/find/findAllAgent",
        method: 'post',
        headers: {
          token: localStorage.getItem("token")
        },
        ...
        if(res.data.code === 200){
          ...
          localStorage.setItem("agent",JSON.stringify(res.data.data.agent))
          this.total = res.data.data.agentLength
          this.loadData()
    ...
    loadData(){
      ...
      this.tableData = JSON.parse(localStorage.getItem("agent"));
  ...
  computed: {
    filterEducation(){
      return function (){
        let obj = [];
        //找到对应的数据 并添加到obj
        this.tableData.forEach(function(agent) {
          if (!obj.some(function(item) {
            return item.text === agent.education;
          })) {
            obj.push({
              text:agent.education,
    ...
    filterAgentName(){
      return function (){
        ...
        this.tableData.forEach(function(agent) {
          if (!obj.some(function(item) {
            return item.text === agent.agentName;
          })) {
            obj.push({
